...
export default class LocationResponseTrigger extends BaseTrigger {
  ...
  private async withinFreeLimit(): Promise<boolean> {
    ...
    return await this.todaysRequests() <= parseInt(process.env.REQUEST_LIMIT, 10);
  }

  private async todaysRequests(): Promise<number> {
    const today = moment().startOf('day');
    const tomorrow = moment(today).endOf('day');

    return await MapsRequest.find({
      requestDate: {
        $gte: today.toDate(),
        $lt: tomorrow.toDate(),
      }
  ...
  private async countRequest(requestor) {
    ...
    console.log(`Request count for today: ${await this.todaysRequests()}`);
  }
